...
async def handle_product_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Créer le nouveau média avec un timestamp comme index
    new_media = {
        ...
        'order_index': int(datetime.now().timestamp())  # Utiliser un timestamp comme index
    }
    ...
    # Ajoutez ces gestionnaires pour la navigation entre les médias
            ...
            try:
                ...
                if product and 'media' in product:
                    # Trier la liste des médias par order_index mais en ordre DÉCROISSANT pour "next"
                    media_list = sorted(product['media'], 
                                     key=lambda x: x.get('order_index', 0),
                                     reverse=True)  # Ajout de reverse=True
                    total_media = len(media_list)
