...
async def handle_product_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Modifier la création du new_media
    timestamp_key = str(int(datetime.now().timestamp() * 1000))  # Ajout du timestamp ici
    new_media = {
        ...
        'timestamp': timestamp_key,  # Stocker le timestamp
        ...
        if category:
            ...
            if product:
                ...
                if 'media' in product and product['media']:
                    media_list = sorted(product['media'], key=lambda x: x.get('timestamp', 0))
