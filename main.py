...
async def handle_product_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('last_message_id'):
        try:
            ...
            # Créer le nouveau média avec un order_index séquentiel au lieu d'un timestamp
            new_media = {
                ...
                'order_index': context.user_data['media_count']  # Utiliser le compteur comme index
            }
    ...
    # Ajoutez ces gestionnaires pour la navigation entre les médias
            ...
            try:
                ...
                if product and 'media' in product:
                    # Trier la liste des médias par order_index de manière croissante
                    media_list = sorted(product['media'], 
                                     key=lambda x: x.get('order_index', 0))  # Supprimé reverse=True
                    total_media = len(media_list)
                    current_index = context.user_data.get('current_media_index', 0)
        
                    # Calculer le nouvel index
                    if direction == "next":
                        current_index = (current_index + 1) % total_media  # Pour aller en avant
                    else:  # prev
                        current_index = (current_index - 1) % total_media  # Pour aller en arrière
                    ...
                    current_media = media_list[current_index]  # Utiliser la liste triée
            
                    # Avant d'accéder au média courant
